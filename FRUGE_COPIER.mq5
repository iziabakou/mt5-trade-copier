...
input string input_accounts="0"; //Accounts allowed
input int delay_timout=60; //Ignore trade older than (s)
string account;
string accounts[];
CTrade trader;
...
//Open new positions
   ...
   for(int i=0; i<ArraySize(provider_pos); i++) {
      ...
      //nb seconds since position open
      int since_s = (int)(TimeGMT()-p.time);

      if(since_s<=delay_timout) {
         if(p.type==POSITION_TYPE_BUY)
            trader.Buy(p.volume,p.symbol,0, p.stop_loss, p.take_profit, comment);
         else if(p.type==POSITION_TYPE_SELL)
